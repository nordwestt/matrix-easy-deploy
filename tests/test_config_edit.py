...
class ConfigEditTests(unittest.TestCase):
    ...
    def test_discover_local_kanidm_sibling(self):
        matrix = self.root / "matrix-easy-deploy"
        kanidm = self.root / "kanidm-easy-deploy"
        matrix.mkdir()
        kanidm.mkdir()
        ...
        with patch.dict(os.environ, {}, clear=False):
            ...
            found = config_edit.discover_local_kanidm(matrix)
        self.assertEqual(found["domain"], "idm.opencomp.eu")
        self.assertTrue(found["deploy"].endswith("kanidm-easy-deploy/deploy.yaml"))

    def test_update_sso_config_sets_kanidm_provider(self):
        config = config_edit.load_or_init(self.deploy_yaml)
        config_edit.update_sso_config(
            config, enabled=True, providers=[], provider="kanidm", default_login="sso"
        )
        self.assertTrue(config["features"]["sso"]["enabled"])
        self.assertEqual(config["features"]["sso"]["provider"], "kanidm")
